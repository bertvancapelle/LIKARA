...
| ADR-055 | De gebruik-verfijning ("welke afdeling gebruikt dit") is component-breed: `ondersteunt_werk` (ADR-045) is de grens, niet het type `applicatie` — herziet een ongegronde applicatie-binding die een overblijfsel bleek van de opgeheven `applicatie`-subtabel; geen bepaling verandert (signaal/norm/snapshot/contracttoets ongewijzigd), alleen wie de vraag kán beantwoorden; geen schemawijziging; signaal 12 → 9 componenten (correctie: die drie waren onbeantwoordbaar) | Besloten (LI047) — bouw volgt |
