...
| ADR-056 | Vragen evolueren: de formulering wordt bevroren bij het antwoord — de beheerder kiest zelf verduidelijking vs. echte wijziging (nooit uit de tekst afgeleid; zelfde keuze bij optie-uitbreiding én label-hernoemen, ook afgeleide sets), verouderd leest neutraal (nooit amber/rood) en is per formulering wegwerkbaar ("mijn antwoord klopt nog"), oude formulering in de geschiedenis, telt mee in Open punten (niet-blokkerend blok), totaalbeeld + voorspelling-vóór-opslaan in het vraagbeheer-scherm uit één telling; mockup-besluiten 13–16: één opslaan-moment per vraag (keuze één keer, met opsomming + telling), antwoordtype-slot aan de bron (veld dicht mét reden), uitgezette optie zichtbaar én heractiveerbaar (domeinbesluit — heractiveren bestaat nog niet), geen voorgeselecteerde keuze in het opslaan-venster; invariant: nooit blok "dit moet nog", nooit de score legen; verworpen: vraagversies, audit als leeslaag | Voorstel — besloten (LI050); bouw volgt, sliceverdeling apart |
