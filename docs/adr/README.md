...
| ADR-052 | Tenant-norm voor harde componentfeiten + verrijkte migratieklaar-verklaring: per hard feit verplicht-ja/nee (MVP, geen weging), "compleet = vastgesteld ≠ gevuld" (sentinel telt niet mee), "bewust geen" voor koppelingen/contract, norm = lat niet poort, bevestiging + auditeerbaar akkoord alléén bij afwijking, badge (live) + log (snapshot) uit één norm-definitie; engine ongemoeid (smalle MVP-voorloper van de beoordelingsgrondslag LI043-1) | Aanvaard — bouw volgt |
