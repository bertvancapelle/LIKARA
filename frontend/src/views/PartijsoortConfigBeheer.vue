<script setup>
/**
 * PartijsoortConfigBeheer — platform-beheer van de partijsoort-catalogus.
 * Schil op `/platform/partijsoortconfig`. Enkel-doel (één platte lijst, geen dimensie);
...
async function laad() {
  ...
  fout.value = null
  try {
    opties.value = await api.platformPartijsoortconfig.lijst()
  } catch (e) {
...
async function bevestigToevoegen() {
  ...
  try {
    ...
    const optie = await api.platformPartijsoortconfig.maak(data)
...
async function bevestigBewerken() {
  ...
  try {
    ...
    const updated = await api.platformPartijsoortconfig.werkBij(editOptie.value.id, data)
    _vervang(updated)
...
async function bevestigDeactiveren() {
  bezig.value = true
  try {
    const updated = await api.platformPartijsoortconfig.werkBij(deactOptie.value.id, { actief: false })
    _vervang(updated)
...
async function reactiveer(optie) {
  try {
    const updated = await api.platformPartijsoortconfig.werkBij(optie.id, { actief: true })
    _vervang(updated)
    ...
    <Dialog v-model:visible="addOpen" modal :closable="false" header="Partijsoort-optie toevoegen" data-testid="ps-add-dialog">
      <form class="flex flex-col gap-[var(--lk-space-md)] min-w-[22rem]" data-testid="ps-add-form" @submit.prevent="bevestigToevoegen">
        ...
        <div class="flex flex-col gap-[var(--lk-space-xs)]">
          ...
          <input id="ps-add-sleutel" v-model="addForm.optie_sleutel" type="text" data-testid="ps-add-sleutel" :aria-invalid="!!addFouten.optie_sleutel" placeholder="bv. ketenpartner" class="lk-veld font-mono" />
        ...
        <div class="flex flex-col gap-[var(--lk-space-xs)]">
          ...
          <input id="ps-add-label" v-model="addForm.label" type="text" data-testid="ps-add-label" :aria-invalid="!!addFouten.label" class="lk-veld" />
        ...
        <div class="flex flex-col gap-[var(--lk-space-xs)]">
          ...
          <input id="ps-add-volgorde" v-model="addForm.volgorde" type="number" data-testid="ps-add-volgorde" placeholder="leeg = achteraan" class="lk-veld w-32" />
        </div>
    ...
    <Dialog v-model:visible="editOpen" modal :closable="false" header="Optie bewerken" data-testid="ps-edit-dialog">
      <form class="flex flex-col gap-[var(--lk-space-md)] min-w-[22rem]" data-testid="ps-edit-form" @submit.prevent="bevestigBewerken">
        ...
        <div class="flex flex-col gap-[var(--lk-space-xs)]">
          ...
          <input id="ps-edit-label" v-model="editForm.label" type="text" data-testid="ps-edit-label" :aria-invalid="!!editFouten.label" class="lk-veld" />
        ...
        <div class="flex flex-col gap-[var(--lk-space-xs)]">
          ...
          <input id="ps-edit-volgorde" v-model="editForm.volgorde" type="number" data-testid="ps-edit-volgorde" class="lk-veld w-32" />
        </div>
        <div class="flex gap-[var(--lk-space-md)]">
