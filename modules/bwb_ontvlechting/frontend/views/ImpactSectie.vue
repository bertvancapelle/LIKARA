...
<template>
  <section class="card" aria-labelledby="sectie-impact">
    ...
    <p v-if="!geladen && !laden" data-testid="im-uitleg" class="text-[length:var(--cd-text-sm)] text-[var(--cd-color-text-muted)]">
    ...
    <template v-if="geladen && data">
      ...
      <p data-testid="im-contracten" class="mb-[var(--cd-space-sm)] text-[length:var(--cd-text-sm)]">
        ...
        <span v-else class="text-[var(--cd-color-text-muted)]"> geen.</span>
      </p>

      <!-- Samenvatting -->
      <p data-testid="im-samenvatting" class="mb-[var(--cd-space-sm)] text-[length:var(--cd-text-sm)]">
      ...
      <DataTable v-if="data.geraakt.length" :value="data.geraakt" data-testid="im-tabel">
        <Column header="Component">
          <template #body="{ data: r }">
            <router-link :to="rijRoute(r)" class="text-[var(--cd-color-primary)] hover:underline">{{ r.naam }}</router-link>
          </template>
        </Column>
        ...
        <Column header="Pad">
          <template #body="{ data: r }">
            <span v-if="r.niveau > 1" :data-testid="`im-pad-${r.component_id}`" class="text-[length:var(--cd-text-xs)] text-[var(--cd-color-text-muted)]">{{ r.pad.join(' → ') }}</span>
            <span v-else>—</span>
          </template>
        </Column>
      </DataTable>
      <p v-else data-testid="im-leeg" class="text-[var(--cd-color-text-muted)] text-[length:var(--cd-text-sm)]">
