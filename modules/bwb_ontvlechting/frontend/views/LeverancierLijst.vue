<script setup>
/**
 * LeverancierLijst — overzicht van leveranciers (ADR-020 contractregister).
 *
 * Spiegelt ApplicatieLijst: DataTable + server-side sorteerbare keyset-paginering
 * (ADR-017), `zoek`-filter (CD041-lijst-API), "Meer laden", detail-link op de naam.
 * Puur registratief — geen lifecycle/status.
 */
...
const heeftFilters = computed(() => !!filterZoek.value.trim())
...
async function laad({ reset = false } = {}) {
  ...
  try {
    ...
    const pagina = await api.leveranciers.lijst(params)
    items.value = reset ? pagina.items : items.value.concat(pagina.items)
    cursor.value = pagina.volgende_cursor
  } catch (e) {
    fout.value = e?.message || 'Er ging iets mis bij het laden van de leveranciers.'
  } finally {
    laden.value = false
...
<template>
  <section aria-labelledby="leveranciers-titel">
    ...
    >
      <label class="flex flex-col gap-[var(--cd-space-xs)] text-[length:var(--cd-text-sm)]">
        ...
        <input
          ...
          aria-label="Zoek op leveranciernaam"
          placeholder="zoeken…"
    ...
    <DataTable
      ...
      data-testid="leveranciers-tabel"
    ...
    >
      <Column field="naam" header="Naam" sortable>
        <template #body="{ data }">
          <router-link
            :to="{ name: 'leverancier-detail', params: { id: data.id } }"
      ...
      <template #empty>
        <span v-if="eersteGeladen && !laden && heeftFilters" data-testid="lijst-geen-match">
          Geen leveranciers komen overeen met de zoekterm.
        </span>
        <span v-else-if="eersteGeladen && !laden" data-testid="lijst-leeg">
          Er zijn nog geen leveranciers in deze tenant.
        </span>
