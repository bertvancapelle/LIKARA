"""Pydantic v2-schemas — platform-beheer van de BIV-schaal-catalogus (ADR-028 slice 1).

Enkel-doel catalogus (geen `dimensie`), spiegel van `schemas/partijsoortconfig`.
`optie_sleutel` is immutable — ontbreekt in Update. Soft-deactivate (geen V). ORDINAAL:
`volgorde` draagt de rangorde (laag<midden<hoog); nieuwe niveaus krijgen hun plek via `volgorde`.
"""
import re
...
class BivSchaalOptieCreate(BaseModel):
...
class BivSchaalOptieUpdate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class BivSchaalOptieRead(BaseModel):
