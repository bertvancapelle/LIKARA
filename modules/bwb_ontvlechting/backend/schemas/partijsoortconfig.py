"""Pydantic v2-schemas — platform-beheer van de partijsoort-catalogus.

Enkel-doel catalogus (geen `dimensie`), spiegel van `schemas/vraagbetekenisconfig`.
`optie_sleutel` is immutable — ontbreekt in Update. Soft-deactivate (geen V).
NB: dit is de bestaande PLATFORM-soortenlijst beheerbaar maken; tenant-eigen partijsoort blijft geparkeerd.
"""
import re
...
class PartijsoortOptieCreate(BaseModel):
...
class PartijsoortOptieUpdate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class PartijsoortOptieRead(BaseModel):
