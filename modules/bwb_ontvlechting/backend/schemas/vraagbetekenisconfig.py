"""Pydantic v2-schemas — platform-beheer van de vraagbetekenis-catalogus.

Enkel-doel catalogus (geen `dimensie`), spiegel van `schemas/relatiekenmerkconfig`.
`optie_sleutel` is immutable (stabiele sleutel) — ontbreekt in Update. Soft-deactivate (geen V).
"""
import re
...
class VraagBetekenisOptieCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")

    optie_sleutel: str
    label: str
    volgorde: int | None = None  # None ⇒ service plaatst achteraan
...
class VraagBetekenisOptieUpdate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class VraagBetekenisOptieRead(BaseModel):
